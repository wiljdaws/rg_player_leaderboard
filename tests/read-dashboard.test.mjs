...
function fullData() {
  return {
    ...
    aggregate: {
      ...
      byHudVersion: { "18.1": 500, "17.6": 100, "17.4": 30 },
      byLabel: {
        site: [
...
test("renderReadDashboard: highlights current HUD release (18.1) in green", async () => {
  installFakeDom();
  try {
    const { renderReadDashboard } = await import("../js/read-dashboard.js");
    const container = makeContainer();
...
test("quota helpers: thresholds + worstTone", async () => {
  installFakeDom();
  try {
    const { quotaTone, worstTone, DAILY_READ_QUOTA } = (await import("../js/read-dashboard.js")).__private;
    assert.equal(DAILY_READ_QUOTA, 50000, "spark plan daily cap is 50k");
    assert.equal(quotaTone(0.2), "ok");
    assert.equal(quotaTone(0.59), "ok");
    assert.equal(quotaTone(0.6), "warn");
    assert.equal(quotaTone(0.79), "warn");
    assert.equal(quotaTone(0.8), "crit");
    assert.equal(quotaTone(1.4), "crit");
    assert.equal(quotaTone(NaN), "ok");
    // worst-of-two picks the more alarming tone.
    assert.equal(worstTone("ok", "warn"), "warn");
    assert.equal(worstTone("warn", "crit"), "crit");
    assert.equal(worstTone("crit", "ok"), "crit");
...
test("todayQuotaSnapshot: null when monitoring missing / today outside range", async () => {
  installFakeDom();
  try {
    const { todayQuotaSnapshot } = (await import("../js/read-dashboard.js")).__private;
    // Monitoring off → null.
    assert.equal(todayQuotaSnapshot({}), null);
    assert.equal(todayQuotaSnapshot({ monitoring: { available: false, byDate: {} } }), null);
    // Monitoring on but today's key not present → null (historic range).
    const snap = todayQuotaSnapshot({
      monitoring: {
        available: true,
        byDate: { "1999-01-01": { reads: 100 } },
      },
    });
